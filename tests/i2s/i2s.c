...
void SAI_RxIRQ_Handler(void)
{
  ...
  uint32_t data = 0;
  uint16_t inc_buf = 0;
  uint8_t bytesPerWord = format->bitWidth / 8U;
  ...
  if (I2S0->RCSR & I2S_RCSR_FRF_MASK){
    ...
    if (SAI_RxGetStatusFlag(I2S0) & kSAI_FIFOWarningFlag)
    {
      ...
      {
        PRINTF("IRQ handler for 1 \r\n");
        for (int k = 0; k < sizeof(temp) / bytesPerWord; k++)
        {
          PRINTF("IRQ handler for 2\r\n");
          data = I2S0->RDR[format->channel];
          PRINTF("IRQ handler for 3\r\n");
          for (j = 0; j < bytesPerWord; j++)
          {
            temp[inc_buf] = (data >> (8U * j)) & 0xFF;
            inc_buf++;
            g_index++;
          }
        }
