...
//  modem_enable();
  if (!modem_enable()) return false;

//  modem_register(timeout);
  if (!modem_register(timeout)) return false;

  if (!modem_gprs_attach(apn, user, password, timeout)) return false;
...
bool modem_mqtt_send(const char *data, uint8_t len)
{
  ...
  if (modem_expect("ERROR", 1000)) return false;

  modem_send("AT+QISEND=%d", len);
  delay(200);
  //it is not recognizing the char so we are using delay here, ths should be fixed
